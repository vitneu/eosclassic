...
const (
	...
	DefaultHTTPPort = 8282        // Default TCP port for the HTTP RPC server
	DefaultWSHost   = "localhost" // Default host interface for the websocket RPC server
	DefaultWSPort   = 8285        // Default TCP port for the websocket RPC server
)
...
func DefaultDataDir() string {
	...
	if home != "" {
		if runtime.GOOS == "darwin" {
			return filepath.Join(home, "Library", "EOSClassic")
		} else if runtime.GOOS == "windows" {
			return filepath.Join(home, "AppData", "Roaming", "EOSClassic")
		} else {
			return filepath.Join(home, ".eosclassic")
		}
	}
